...
if ($result = mysql_query($sql2))
{
	if (@mysql_num_rows($result) > 0)
	{
		...
		{
			$userlist .= '<tr class="style2"><td>'.$row['lastname'].', '.$row['firstname'].'</td><td>'.$row['username'].'</td><td>'.str_chop($row['email'],20).'</td><td><a href="./userinfo.php?user='.$row['username'].'"><img src="images/hwinfo.gif" alt="Info" border="0" title="More Information" /></a> <a href="./bulkemail.php?user='.$row['username'].'"><img src="images/email.gif" alt="Email" border="0" title="Email" /></a> <a href="#" onclick="deleteuser(\''.$row['username'].'\')"><img src="images/editdelete.gif" alt="Remove" border="0" title="Remove" /></a></tr>'."\n";
		}
	}
	else
	{

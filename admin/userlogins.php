...
$sql = 'SELECT * FROM deadlock_logins WHERE `username` = \''.$loginusername.'\'';
...
if ($result = mysql_query($sql2))
{
	if (@mysql_num_rows($result) > 0)
	{
		...
		{
			$stats .= '<tr class="style2"><td>'.$row['username'].'</td><td>'.date ('d M Y H:i',$row['timestamp']).'</td><td>'.$row['ip'].'</td></tr>'."\n";
		}
	}
  ...
  <tr>
    <td height="19" colspan="2"><br />
      ...
      <table width="500" border="0" align="center">
        <tr>
          <td width="30%" class="style5">Korisnik</td>
          <td width="40%" class="style5">Vrijeme</td>
          <td width="30%" class="style5">IP adresa</td>
